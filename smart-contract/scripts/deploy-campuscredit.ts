...
async function main() {
  console.log(
    "🚀 Starting CampusCredit token deployment to Monad Testnet...\n"
  );
  ...
  // Deploy CampusCredit contract
  console.log("📦 Deploying CampusCredit token contract...");
  const CampusCreditFactory = await ethers.getContractFactory("CampusCredit");

  // Estimate gas
  const deployTx = await CampusCreditFactory.getDeployTransaction();
  const estimatedGas = await ethers.provider.estimateGas(deployTx);
  console.log("├── Estimated gas:", estimatedGas.toString());

  // Deploy with manual gas limit (adding 20% buffer)
  const gasLimit = (estimatedGas * BigInt(120)) / BigInt(100);
  const campusCredit: CampusCredit = await CampusCreditFactory.deploy({
    gasLimit: gasLimit,
  });
  ...
  console.log("✅ CampusCredit token deployed successfully!");
